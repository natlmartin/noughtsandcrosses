...
function drawBoard() {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    cell.addEventListener('click', function()) {
        ...
        state.cells[i] = state.symbols[O]

        drawBoard()

        if (checkForWinner('Congratulations! You Won')) {
            state.gameFinished = true
            drawMessage()
        }

        if (checkForDraw("It's a draw!")) { // use double quotation if string contains speech mark
            state.gameFinished = true
            drawMessage()
        }
    })
